...
export class MenuService {
  ...
  GetMenu(request: ServerQueryRequest): Observable<ServerQueryResponse<IMenu>> {
    return this.http.get<ServerQueryResponse<IMenu>>(
      `${environment.API_URL}${GlobalConstant.API_END_POINTS.Menu}?GlobalSearch=${request.globalSearch}&PageIndex=${request.page}&PageSize=${request.pageSize}`,
    );
  }
  ...
  updateMenu(id: number, data: IMenu): Observable<IApiResponse> {
    return this.http.put<IApiResponse>(
      `${environment.production}${GlobalConstant.API_END_POINTS.updateMenu}/${id}`,
      data,
    );
  }
  ...
  deleteMenu(id: number): Observable<IApiResponse> {
    return this.http.delete<IApiResponse>(
      `${environment.production}${GlobalConstant.API_END_POINTS.deleteMenu}/${id}`,
    );
  }
